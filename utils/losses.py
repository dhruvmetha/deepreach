...
def init_temporal_consistency_loss():
    def temporal_consistency_loss(v_curr, dvdt, dvds, tc_obs_flow, boundary_values):
        flow_dot = torch.sum(dvds * tc_obs_flow, dim=-1)           # ∇V · ẋ_obs (= Hamiltonian for no-control)
        diff_constraint_hom = dvdt - flow_dot                       # dV/dτ - H
        diff_constraint_hom = torch.max(diff_constraint_hom, v_curr - boundary_values)  # BRT operator
        return {'diff_constraint_hom': torch.abs(diff_constraint_hom).sum()}
